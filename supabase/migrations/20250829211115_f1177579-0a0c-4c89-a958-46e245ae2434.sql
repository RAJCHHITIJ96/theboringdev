-- Add new status values for quality and publishing stages
-- Update zuhu_content_processing with new status values and quality audit relationship
ALTER TABLE zuhu_content_processing ADD COLUMN IF NOT EXISTS quality_audit_id UUID REFERENCES quality_audits(id);

-- Update status enum by recreating the table with expanded statuses (safe migration)
-- First create new table with expanded statuses
CREATE TABLE zuhu_content_processing_new (
  ...
  status VARCHAR NOT NULL DEFAULT 'received' CHECK (status IN (
    'received', 'processing', 'classified', 'design_approved', 'assets_processed', 
